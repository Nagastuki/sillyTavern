...
`;
  ...
  function renderChat(friend) {
    ...
    return `<div class="pm-chat">
      <div class="pm-chat-head"><button class="pm-icon-btn" data-action="back">‹</button><div class="pm-avatar" style="width:34px;height:34px;border-radius:10px">${avatarText(friend.name)}</div><div class="pm-chat-title">${escapeHtml(friend.name)} <span class="pm-muted">· v${VERSION} · ${runtime.settings.generationMode === 'preset' ? '预设兼容' : '隔离生成'}</span>${friend.muted ? '<span class="pm-muted"> · 已静音</span>' : ''}</div></div>
