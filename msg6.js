...
(() => {
  ...
  const VERSION = '0.2.4';
  const BOOT_MARKER_ID = 'letter-early-boot-marker';
  let earlyBootDocument = document;
  let earlyBootWindow = window;
  try {
    for (let depth = 0; depth < 8; depth += 1) {
      const parentWindow = earlyBootWindow.parent;
      if (!parentWindow || parentWindow === earlyBootWindow) break;
      // 读取 document 是同源检查；跨域时会在此处抛错并停止向上。
      if (!parentWindow.document) break;
      earlyBootWindow = parentWindow;
      earlyBootDocument = parentWindow.document;
    }
  } catch { /* cross-origin iframe */ }
  try {
    const reached = { version: VERSION, at: new Date().toISOString(), href: location.href };
    window.__LETTER_IMPORT_REACHED__ = reached;
    try { earlyBootWindow.__LETTER_IMPORT_REACHED__ = reached; } catch { /* cross-origin iframe */ }
    earlyBootDocument.getElementById(BOOT_MARKER_ID)?.remove();
    const marker = earlyBootDocument.createElement('button');
    marker.id = BOOT_MARKER_ID;
    marker.type = 'button';
    marker.textContent = `信笺启动中 · v${VERSION}`;
    marker.style.cssText = 'position:fixed;left:12px;bottom:12px;z-index:2147483647;padding:8px 12px;border:1px solid #78939b;border-radius:10px;background:#edf3f3;color:#35515b;box-shadow:0 4px 18px rgba(20,40,48,.28);font:12px sans-serif';
    (earlyBootDocument.body || earlyBootDocument.documentElement).appendChild(marker);
  } catch (error) {
    console.error('[信笺] 最早期启动标记创建失败', error);
  }
  ...
  function storage() {
    try {
      return runtime.rootWindow?.localStorage || globalThis.localStorage || null;
    } catch (error) {
      warn('浏览器拒绝访问 localStorage，将使用当前页面内存存储。', error);
      return null;
    }
  }
  ...
  function readJson(key, fallback) {
    try {
      ...
      const memoryRaw = runtime.memoryStorage.get(key);
      return raw ? JSON.parse(raw) : memoryRaw ? JSON.parse(memoryRaw) : clone(fallback);
    } catch (error) {
  ...
  function writeJson(key, value) {
    ...
    runtime.memoryStorage.set(key, serialized);
    try {
      storage()?.setItem(key, serialized);
    } catch (error) {
      // Safari 私密模式、受限 WebView 或存储配额不足时不能让整个 UI 初始化失败。
      warn('写入 localStorage 失败；聊天元数据仍会保存，当前页面使用内存回退。', error);
    }
  }
  ...
  function getHost() {
    ...
    const accessibleWindows = [window];
    try {
      for (let depth = 0; depth < 8; depth += 1) {
        const parentWindow = rootWindow.parent;
        if (!parentWindow || parentWindow === rootWindow) break;
        if (!parentWindow.document) break;
        rootWindow = parentWindow;
        accessibleWindows.push(parentWindow);
      }
    } catch (error) {
      warn('更高层页面不可访问，将在当前可访问的最高层窗口中运行。', error);
    }
    let helper = null;
    let silly = null;
    for (const candidate of accessibleWindows) {
      try { if (!helper && candidate.TavernHelper) helper = candidate.TavernHelper; } catch { /* ignore */ }
      try { if (!silly && candidate.SillyTavern) silly = candidate.SillyTavern; } catch { /* ignore */ }
    }
    return { rootWindow, helper, silly, accessibleWindows };
  }
  ...
  function getHelperFunction(...names) {
    const { helper, accessibleWindows } = getHost();
    for (const name of names) {
      if (typeof helper?.[name] === 'function') return helper[name].bind(helper);
      for (const candidate of accessibleWindows) {
        try { if (typeof candidate?.[name] === 'function') return candidate[name].bind(candidate); } catch { /* ignore */ }
      }
    }
    return null;
  }

  function getAccessibleGlobal(name) {
    for (const candidate of getHost().accessibleWindows) {
      try { if (candidate?.[name] !== undefined) return candidate[name]; } catch { /* ignore */ }
    }
    return undefined;
  }
  ...
  async function withoutPromptTemplate(task) {
    const engine = getAccessibleGlobal('EjsTemplate');
  ...
  function subscribeGenerationStream(generationId, onText) {
    ...
    const events = getAccessibleGlobal('iframe_events');
  ...
  function subscribeEvents() {
    ...
    const events = getAccessibleGlobal('tavern_events');
    if (!eventOnFn || !events) {
...
.pm-fab{width:62px;height:62px;border-radius:50%;border:1px solid rgba(105,130,139,.45);background:radial-gradient(circle at 38% 30%,#f8faf7,#dbe7e9 72%,#bfd1d5);color:#35515b;box-shadow:0 8px 30px rgba(40,65,73,.3)}.pm-fab:hover{box-shadow:0 12px 38px rgba(40,65,73,.34)}.pm-envelope{position:relative;width:38px;height:27px;border:1.5px solid #587680;border-radius:4px;background:#f9f6ed;overflow:hidden;box-shadow:0 2px 6px rgba(45,70,77,.18)}.pm-envelope:before{content:"";position:absolute;left:0;right:0;top:0;height:19px;background:#dce8ea;clip-path:polygon(0 0,100% 0,50% 78%);border-bottom:1px solid rgba(85,115,124,.24)}.pm-envelope:after{content:"";position:absolute;left:50%;bottom:-16px;width:31px;height:31px;border:1px solid rgba(88,118,128,.48);background:#f9f6ed;transform:translateX(-50%) rotate(45deg)}.pm-envelope i{position:absolute;left:50%;top:15px;z-index:2;width:9px;height:9px;border-radius:50%;background:#a35852;box-shadow:0 0 0 2px rgba(249,246,237,.88);transform:translate(-50%,-50%)}
...
`;
  ...
  function render() {
    ...
    root.innerHTML = `<button class="pm-fab" aria-label="信笺"><span class="pm-envelope" aria-hidden="true"><i></i></span>${totalUnread() ? `<span class="pm-badge">${totalUnread()}</span>` : ''}</button>
  ...
  async function init() {
    ...
    render();
    try {
      const marker = earlyBootDocument?.getElementById(BOOT_MARKER_ID);
      if (marker) {
        marker.textContent = `信笺已启动 · v${VERSION}`;
        marker.style.borderColor = '#5b9a70';
        marker.style.color = '#326044';
        runtime.rootWindow.setTimeout(() => marker.remove(), 2600);
      }
    } catch { /* ignore diagnostic marker */ }
    subscribeEvents();
  ...
  async function destroy() {
    ...
    try { earlyBootDocument?.getElementById(BOOT_MARKER_ID)?.remove(); } catch { /* ignore */ }
    if (runtime.rootWindow?.[APP_KEY]?.destroy === destroy) delete runtime.rootWindow[APP_KEY];
  }

  function exposeBootstrapError(error) {
    const message = describeGenerationError(error);
    try { window.__LETTER_BOOT_ERROR__ = { message, stack: asText(error?.stack), at: new Date().toISOString() }; } catch { /* ignore */ }
    try {
      const doc = runtime.document || document;
      const earlyMarker = doc.getElementById(BOOT_MARKER_ID);
      const marker = earlyMarker || doc.createElement('button');
      marker.id = earlyMarker ? BOOT_MARKER_ID : 'letter-bootstrap-error';
      marker.textContent = '信笺加载失败';
      marker.title = message;
      marker.style.cssText = 'position:fixed;right:12px;bottom:12px;z-index:2147483647;padding:8px 12px;border:1px solid #8f4d49;border-radius:10px;background:#f7e9e5;color:#713f3b;font:12px sans-serif';
      marker.onclick = () => alert(`信笺初始化失败：\n${message}`);
      if (!earlyMarker) (doc.body || doc.documentElement).appendChild(marker);
    } catch { /* ignore */ }
  }

  const ready = () => init().catch((error) => {
    console.error('[信笺] 初始化失败', error);
    exposeBootstrapError(error);
