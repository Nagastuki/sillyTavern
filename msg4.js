...
(() => {
  ...
  async function runFriendGeneration(friend, userMessage, options = {}) {
    ...
    try {
      ...
      assistantMessage.content = `${options.appendPrefix || ''}${reply}`;
...
`;
  ...
  function renderOneMessage(message, friend) {
    ...
    return `<div class="pm-message ${message.role} ${failed ? 'failed' : ''}" data-message-id="${message.id}"><div class="pm-bubble">${quote}<div class="pm-message-content" data-message-content="${message.id}">${renderMessageContent(message.content || (message.status === 'streaming' ? '…' : ''))}</div><div class="pm-message-meta">${message.proactive ? '主动消息 · ' : ''}${formatTime(message.createdAt)}${messageStatusText(message) ? ` · ${escapeHtml(messageStatusText(message))}` : ''}</div>${actions}</div></div>`;
  }
  ...
  async function resizeAvatar(file) {
    ...
    const image = await new Promise((resolve, reject) => {
      const node = new Image(); node.onload = () => resolve(node); node.onerror = () => reject(new Error('图片格式无效。')); node.src = dataUrl;
    });
  ...
  async function uploadFriendAvatar(friendId, file) {
    const friend = getFriend(friendId);
    ...
    } catch (error) { notify(`头像上传失败：${error.message || error}`, 'error'); }
  }
  ...
  function bindUiEvents(root) {
      ...
      element.addEventListener('click', async (event) => {
        ...
        if (action === 'clear-letter-data') {
          if (runtime.rootWindow.confirm('清空当前聊天中的全部信笺好友、消息与测试投影？此操作不能撤销。')) {
            ...
            await clearPigeonProjections(); runtime.chatData = createEmptyChatData(); saveChatData(); runtime.activeFriendId = ''; render();
          }
        }
      });
    });
    ...
    avatarInput?.addEventListener('change', () => uploadFriendAvatar(runtime.pendingAvatarFriendId || runtime.activeFriendId, avatarInput.files?.[0]));
